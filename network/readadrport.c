...
void addrport_read(SAPP result, const char *input){
	int len = 0;
	int idx = 0;
	int i;
	int cc = 0;
...
int main_readadrport(const char* progname, const int argc, const char **argv){
	SAPP res;
	int i = 0;

    if(argc<2) help_readadrport();

	res = (SAPP)malloc(sizeof(SAPP));
	addrport_read(res, argv[1]);
