...
void help_ipv6hexread(){
	printf("\n");
	printf("ipv6hexread\n");
	printf("Recebe um endereco IPv6 e imprime os bytes em hexadecimal\n");
	printf("\n");
	printf("Autor: Patrick Brandao <patrickbrandao@gmail.com>, todos os direitos reservados\n");
	printf("\n");
	printf("Use: ipv6hexread (ipv6)\n");
	printf("\n");
	exit(1);	
}

int main_ipv6hexread(const char *progname, const int argc, const char **argv){
    struct sockaddr_in6 sa;
    char str[INET6_ADDRSTRLEN];
    if(argc<2 || argc > 2) help_ipv6hexread();
